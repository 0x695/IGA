 ...
 * open-source engine that covers it: Akhenaten's building configs for
 * Pharaoh, eZeus's building source for Zeus, Julius's building/industry.c
 * and resource.c for Caesar III. Emperor's numbers exist only as a
 * walkthrough author's estimates, and Caesar and Caesar II have no engine
 * that carries them, so none of the three are here.
 *
 * The three that are here are not one shape forced onto three games -
 ...
 * progressMax for Caesar III, whose per-day rate is however many workers are
 * currently employed rather than a fixed number.
 */
