 ...
 *
 * Generated by scripts/build-buildings.mjs, one section per game, each
 * joining that game's own source by numeric index or literal constructor
 * argument and checked against independently-known sizes. Edit the script,
 * not this data.
 *
 * `cost` is null for Caesar III and Zeus - not in the engine at all. Caesar
 * III reads it at runtime from c3_model.txt, which ships with the game and
 * which mods rewrite, so there is no single correct value to publish; the
 * format page for that file is where a reader is sent instead. Zeus's own
 * source (eZeus) was searched for an equivalent and none was found, which is
 * itself worth stating rather than leaving cost silently absent with no
 * explanation. Pharaoh is the exception: Akhenaten carries cost in its own
 * building definitions, one number per difficulty level.
 *
 * `employees` is null for Caesar III for the same c3_model.txt reason. Zeus
 * carries it directly in most constructors (`maxEmployees`, the argument
 * right after a building's width and height) and it is published wherever
 * that argument could be read unambiguously; null where the building has no
  ...
  schema: z.object({
    ...
    variants: z.array(
      z.object({
        ...
        engineType: z.string(),
        /**
         * Footprint in tiles. Square buildings are one number; a handful of
         * Zeus buildings (gatehouses, the palace, the stadium, the two agora
         * types) rotate between two fixed footprints and are neither square
         * nor pinned to one orientation, so those carry `[width, height]`
         * instead - the unrotated reading, with the alternate noted in
         * `notes`. Null where not applicable.
         */
         ...
         * this is an array rather than a scalar. Null for Caesar III and
 ...
 *
 * The games here are documented to different depths and the schema does
 ...
 * Zeus's names and thresholds come from two sources that check each other.
 * Caesar II is the one ladder built from two sources that, alone, would each
 * have been too thin to publish - the reconstruction's house_gfxdat table has
 * sizes but no English names, the manual's Appendix has 31 names in order but
 * no sizes - and turned out to agree on every point where they overlap.
 */
 ...
 * scripts/build-production.mjs, one section per game, each read from the
 * best source that covers it: Akhenaten's building configs for Pharaoh,
 * eZeus's building source for Zeus, Julius's building/industry.c and
 * resource.c for Caesar III, the manual's own Appendix for Caesar II (whose
 * decompilation never compiled business names into the binary at all -
 * they lived in the external text resource). Emperor's numbers exist only
 * as a walkthrough author's estimates, and Caesar has no engine or manual
 * appendix that carries them, so neither is here.
 *
 * None of the four here are one shape forced onto four games -
 * productionRate/progressMax/rateByDifficulty are filled in exactly as far
 * as each source actually supports and no further (see the file-level
 * comment in the script): all three for Pharaoh, none for Zeus, only
 * progressMax for Caesar III (whose per-day rate is however many workers
 * are currently employed rather than a fixed number), none for Caesar II
 * (whose only source is prose, not a timed model at all). Caesar II's rows
 * are also fewer than its source's business list: eleven more businesses are
 * named in the manual with a stated raw material, but not a stated output,
 * and are left out rather than guessed - see the production page's caveat.
 */
